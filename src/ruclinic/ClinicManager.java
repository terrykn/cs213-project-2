...
public class ClinicManager {
    ...
    public int NEXTTECHNICIANINDEX = 0; // index of the next technician to check for availability in rotation list
    ...
    private void loadProvidersList() {
        try {
            File file = new File("src/ruclinic/providers.txt");
     ...
     */
    public String formatMoneyString(double amount) {
     ...
     */
    public boolean isValid_ApptDate(Date date) {
        if (!date.isValid()) {
     ...
     * @return true if valid, false otherwise
     */
    public boolean isValid_DOB(Date date) {
        if (!(date.isValid())) {
